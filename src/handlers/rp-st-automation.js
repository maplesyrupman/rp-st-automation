...
async function uploadReportToST(file, jobId, stAuthToken) {
    const formData = new FormData();
    formData.append('file', fileBuffer, { filename: 'test.pdf', contentType: 'application/pdf' });
...
export const handler = async (event, context) => {
    ...
    const filePath = new URL('./test.pdf', import.meta.url);
    const file = readFileSync(filePath, 'base64');
    const {data} = await uploadReportToST(file, 23080135, token)
    console.log(data)
    ...
    try {
        ...
        // const preReportData = await getPreReportData(connection)
